...
def optimize_weights(model, input_dict_data, response_data, training_settings):
    ...
    for key in list(input_dict_data.keys()):
        train_data_torch = np.float64(
            np.append(train_data_torch, input_dict_data[key].training_data, axis=1))
    train_data_torch = np.float64(
        np.append(train_data_torch, response_data.training_data, axis=1))

    val_data_torch = np.zeros((response_data.validation_data.shape[0], 0))
    for key in list(input_dict_data.keys()):
        val_data_torch = np.float64(
            np.append(val_data_torch, input_dict_data[key].validation_data, axis=1))
    val_data_torch = np.float64(
        np.append(val_data_torch, response_data.validation_data, axis=1))

    batch_size = int(training_settings['batch-size'])
    training_data_loader = torch.utils.data.DataLoader(
        train_data_torch, batch_size=batch_size)
    val_data_loader = torch.utils.data.DataLoader(
        val_data_torch, batch_size=batch_size)
