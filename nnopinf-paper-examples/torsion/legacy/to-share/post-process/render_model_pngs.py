...
def parse_args():
    ...
    p.add_argument("--rom-dims", default="8,12,16,20,40", help="Comma-separated ROM dimensions")
...
def _compute_fom_color_range(args, view, fom_exodus, array_name, times):
    ...
    global_min = None
    global_max = None
    reader = ExodusIIReader(FileName=[fom_exodus])
...
def main():
    ...
    model_runs = [
        ...
        ("OpInf-A", "../linear/linear-opinf-operator-"),
        ("OpInf-AH", "../quadratic/quadratic-opinf-operator-"),
        ("NN-OpInf-NN", "../nn-forward/nn-operator-"),
    ]
    fom_exodus = "../fom/torsion.e"
    ...
    if args.disp_mag and array_name is None:
        for label, base_dir in model_runs:
            for rom_dim in rom_dims:
                exodus_path = os.path.join(base_dir + str(rom_dim), "torsion.e")
    ...
    for label, base_dir in model_runs:
        for rom_dim in rom_dims:
            ...
            for time_value in times:
                ...
                render_exodus(args, view, exodus_path, output_path, array_name, color_range, time_value)
    ...
    if os.path.exists(fom_exodus):
        for time_value in times:
            ...
            render_exodus(args, view, fom_exodus, output_path, array_name, color_range, time_value)
    else:
