...
if __name__ == '__main__':
    ...
    for i,truncation_value in enumerate(truncation_values):
      ...
      settings['neural-network-training-settings'] = {'model-name': 'opinf-operator', 'output-path': 'test-3' + str(settings['truncation-value']), 'optimizer': 'ADAM', 'LBFGS-acceleration': True, 'LBFGS-acceleration-epoch-frequency': 5001, 'LBFGS-acceleration-iterations': 50, 'num-epochs': 20000, 'batch-size': 50000, 'learning-rate': 5e-3, 'weight-decay': 1e-7, 'lr-decay': 0.9997, 'print-training-output': True, 'resume': False, 'epoch': 25000, 'hierarchical-update': False,'dynamics-constrained' : False,}# 'ADAM-restart-epoch-frequency':1000,'SR1-restart-epoch-frequency':1000,'TR-delta0': 4000,'TR-cg-tol': 1e-2, 'TR-eta1':1e-6,'TR-eta2':0.5 ,'TR-NEWTON-acceleration':True,'TR-NEWTON-acceleration-epoch-frequency':1000,'TR-NEWTON-acceleration-iterations':50}
