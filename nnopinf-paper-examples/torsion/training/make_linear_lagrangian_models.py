...
if __name__ == '__main__':
    ...
    for i in range(0,len(truncation_sizes)):
        settings['neural-network-training-settings'] = {'model-name': 'opinf-operator', 'output-path': 'ortho-linear-lagrangian-models-' + str(truncation_sizes[i]), 'print-training-output': True, 'resume': False, 'GN-final-layer': False, 'GN-num-layers': 0, 'GN-final-layer-epoch-frequency': 0, 'GN-final-layer-damping': 0.0, 'GN-verbose': False, 'optimizer': {'method': 'ADAM', 'num-epochs': 25000, 'batch-size': 500, 'learning-rate': 5.e-3, 'weight-decay': 1e-06, 'lr-decay': 0.9998, 'LBFGS-acceleration': {'enabled': True, 'acceleration-epoch-frequency': 5001, 'acceleration-iterations': 50}}}
        settings['architecture'] = {'model-structure': 'LinearLagrangianOperator', 'n-hidden-layers': 3, 'n-neurons-per-layer': 'auto'}
