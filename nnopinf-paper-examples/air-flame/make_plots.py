...
#          if model_type != 'ROM': 
  ...
  for j in range(len(model_types)):
    ...
    plt.plot(stop_times,st_errors[best_model_index[j,:,-1],j,ax3vals,-1].flatten(),'-o',color=colors[j],label=display_label_map.get(model_types[j], model_types[j]))
  ...
  for j in range(len(model_types)):
    ...
    if model_types[j] in stochastic_models: 
      plt.errorbar(KsToRun,st_errors[best_model_index[j,-1],j,-1,ax4vals].flatten(),eb,marker=markers[j],ls='-',color=colors[j],label=display_label_map.get(model_types[j], model_types[j]), markersize=11,capsize=10)
    else:
      plt.plot(KsToRun,st_errors[best_model_index[j,-1],j,-1,ax4vals].flatten(),ls='-',marker=markers[j],color=colors[j],markersize=11,label=display_label_map.get(model_types[j], model_types[j]))
  ...
  for j in range(len(model_types)):
    ...
    #else:
    #plt.plot(x,sols[best_model_index[j,-1,-1], :],lt[j],color=colors[j],label=display_label_map.get(model_types[j], model_types[j]))
...
#          else:
  ...
  for j in range(len(model_types)):
    ...
    stochastic_models = ['NN','SS-SPD','Matrix']
    if model_types[j] in stochastic_models: 
      plt.errorbar(KsToRun,st_errors[best_model_index[j,-1],j,-1,ax4vals].flatten(),eb,marker=markers[j],ls='-',color=colors[j],label=display_label_map.get(model_types[j], model_types[j]), capsize=10,markersize=11)
    else:
      plt.plot(KsToRun,st_errors[best_model_index[j,-1],j,-1,ax4vals].flatten(),marker=markers[j],color=colors[j],label=display_label_map.get(model_types[j], model_types[j]),markersize=11)
  plt.xlabel(r'Reduced basis dimension, $K$',**axis_font)
  plt.ylabel(r'Relative error',**axis_font)
  plt.ylim([1e-5,10])
  plt.yscale('log')
  ...
  plt.figure(3)
  x = np.linspace(0,1.8,fom_yaml['nx'])
  y = np.linspace(0,0.9,fom_yaml['ny'])
  fom_sol_to_plot = np.reshape( fom_solution_testing['u'][:,-1,-1],  (4,fom_yaml['nx'],fom_yaml['ny']))
  for j in range(len(model_types)):
    for n in range(0,n_samples):
      model_type = model_types[j]
      K = KsToRun[-1]
    ...
    sol_to_plot = np.reshape( data['u'][:,-1,-1],  (4,fom_yaml['nx'],fom_yaml['ny']))
    print(np.shape(sol_to_plot))
    clevels = np.linspace(0,7*300,30)
    plt.contourf(x,y,300*sol_to_plot[-1].transpose(),20,cmap='jet')
    plt.xlabel(r'$x$',**axis_font)
    plt.ylabel(r'$y$',**axis_font)
    plt.colorbar()
    plt.tight_layout()
    plt.savefig(savePath + 'testing_solution_' + model_types[j] + '.pdf')

  plt.close("all")
  sol_to_plot = np.reshape( fom_solution_testing['u'][:,-1,-1],  (4,fom_yaml['nx'],fom_yaml['ny']))
  clevels = np.linspace(0,7*300,30)
  plt.contourf(x,y,300*sol_to_plot[-1].transpose(),clevels,cmap='jet')
  plt.xlabel(r'$x$',**axis_font)
  plt.ylabel(r'$y$',**axis_font)
  plt.colorbar()
  plt.tight_layout()
  plt.savefig(savePath + 'testing_solution_fom.pdf')
  ...
  for j in range(len(model_types)):
    ...
    print(np.shape(sol_to_plot))
    clevels = np.linspace(0,7*300,30)
    plt.contourf(x,y,np.abs(sol_to_plot[-1].transpose() - fom_sol_to_plot[-1].transpose()) /(fom_sol_to_plot[-1].transpose()),np.linspace(0,0.02,30),cmap='jet')
    plt.xlabel(r'$x$',**axis_font)
    plt.ylabel(r'$y$',**axis_font)
    plt.colorbar()
    plt.tight_layout()
    plt.savefig(savePath + 'testing_solution_error_' + model_types[j] + '.pdf')
