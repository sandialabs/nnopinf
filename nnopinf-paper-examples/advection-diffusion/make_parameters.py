...
if __name__=='__main__':
  ...
  bounds_left = [2.0,1.e-5,np.pi/6,1.0]
  bounds_right = [3.0,1.e-3,np.pi/2.,3.0]
  ...
  if input_yaml_base['sample-corners']:
    ...
    p1,p2,p3,p4 = np.meshgrid(corners[0],corners[1],corners[2],corners[3])
    p1 = p1.flatten()
    p2 = p2.flatten()
    p3 = p3.flatten()
    p4 = p4.flatten()
    ...
    corner_samples = np.append(corner_samples,p3[:,None],axis=1)
    corner_samples = np.append(corner_samples,p4[:,None],axis=1)
